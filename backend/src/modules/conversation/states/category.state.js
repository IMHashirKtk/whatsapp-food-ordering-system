...
export const handle = async (conversation, message) => {
  ...
  // Reload conversation because state/context changed
  const updatedConversation = await conversationService.getConversationById(
    conversation.id,
  );

  // Enter PRODUCT state immediately.
  // listReply is cleared because PRODUCT expects a fresh entry.
  return productState.handle(updatedConversation, {
    ...message,
    listReply: null,
  });
};
